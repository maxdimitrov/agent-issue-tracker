...
3. For each root, derive its display state per its shape:
   - **Evergreen** — from the machine block fetched in step 1 and the union child set (see "Deriving child state"): the phase **label** is the phase containing next-up (or `flat` when there is no phase map); the count next to it is `<closed>/<total>` across ALL of the node's **direct** children, not scoped to the labeled phase alone — same scope as legacy's `<closed>/<total>` below, so the `2/4 direct` column means the same thing for both shapes — plus the machine block's `## Current branch` section, and next-up itself (derived, not stored).
...
2. **Build the machine block.** Assemble the new marker-tagged comment body (see `templates/epic-machine-block.md`) from what the legacy body already carries:
   - `## Phases` — copy the body's `## Phases` section (phase name + ordered refs) verbatim, if present. If the body has NO `## Phases` section, build one from the `## Children` mirror's `(Phase N)` annotations instead (`- [ ] #43 — extract retry-policy into table (Phase 2)`): group refs by `N` ascending, each phase's goal text is just `Phase N` when the legacy body offers nothing better. Only when NEITHER a body `## Phases` section NOR any mirror `(Phase N)` annotation exists is the adopted epic flat — then write no `## Phases` section at all; native return order governs (Fork #4 of the evergreen-epics design).
