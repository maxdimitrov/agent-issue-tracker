...
7. **Plugin loads enabled post-install (added v1.0.2, #37)** — same clean-machine session, `claude plugin list` shows the plugin as `Status: ✔ enabled` (not `✘ failed to load`); `claude plugin details agent-issue-tracker` reports all components (6 skills + 12 commands + the SessionStart hook; current CLIs render skills and commands together, e.g. `Skills (18)`); in a fresh CC session opened against the install, `/tracker-doctor`, `/tracker-init`, and `/resume-initiative` are resolvable as slash commands. Catches `plugin.json.dependencies` cross-marketplace-resolution regressions.
